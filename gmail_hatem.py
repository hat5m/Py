...
def check(email):
  ...
  try:
   ...
   #   print(response.text)
      if 'email_is_taken' in str(response.text):hatem(email)
   ...
   #   print(str(response))
      if '"user":true' in response:hatem(email)
      else:bads_instgram+=1
  except:''
   # check(email)
  os.system('clear' if os.name == 'posix' else 'cls')
...
def qqq():
  while True:
    try:
      ...
      id=str(randrange(128053904,438909537))
      headers = {
      'accept': '*/*',
      'accept-language': 'en-US,en;q=0.9',
      'content-type': 'application/x-www-form-urlencoded',
      ...
      username = requests.post('https://www.instagram.com/api/graphql', headers=headers, data=data).json()['data']['user']['username']#['@lllcxc']
